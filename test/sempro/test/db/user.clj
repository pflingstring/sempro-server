...
(conman.core/with-transaction [t-conn db/conn]
  ...
  (fact "..:: USER ::.."
    ...
    (fact "using handlers.user"
      (facts "return bad request and validation-error message"
        (fact "email-error"
          (new-user-req res/kaput-email) => (contains ((u/error-response err/input-error)
                                                        {:email '("email must be a valid email address")})))
        (fact "password-error"
          (new-user-req res/kaput-pass) => (contains ((u/error-response err/input-error)
                                                       {:pass '("pass is less than the minimum")})))))
